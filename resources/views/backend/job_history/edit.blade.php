        ...
        <div class="row">
            <div class="col-md-12">
                <div class="card card-info">
                    ...
                    <form action="{{ url('admin/job_history/edit/'.$getRecord->id)}}" class="form-horizontal" method="post" enctype="multipart/form-data">
                        {{ csrf_field() }}
                        <div class="card-body">
                                    ...
                                    <select name="employee_id" id="" class="form-control">
                                        ...
                                        @foreach ($getEmployee as $value_employee)
                                            <option {{ $value_employee->id == $getRecord->employee_id ? 'selected' : '' }}
                                                value="{{ $value_employee->id}}">{{ $value_employee->name }} {{ $value_employee->last_name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-10">
                                    ...
                                    <option {{ $getRecord->department_id == '2' ? 'selected' : '' }} value="2">Backend Developer</option>
                                    </select>
                                 </div>
                            </div>
